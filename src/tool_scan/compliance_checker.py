...
class ComplianceChecker:
    ...
    def check(self, tool: Dict[str, Any]) -> ComplianceReport:
        ...
        required_passed = all(
            ...
            for c in checks if c.level == ComplianceLevel.REQUIRED
        )
        ...
        if scorable_checks:
            ...
            passed_weight = sum(
                weights[c.level] for c in scorable_checks
    ...
    def _check_required_fields(self, tool: Dict[str, Any]) -> List[ComplianceCheck]:
        ...
        checks.append(ComplianceCheck(
            ...
            message="Tool name is present" if "name" in tool else "Tool name is missing (required)",
        ...
        checks.append(ComplianceCheck(
            ...
            message="Tool description is present" if has_description else "Tool description is missing",
            spec_reference=f"{self.SPEC_URL}/server/tools",
        ))
        ...
        checks.append(ComplianceCheck(
            ...
            message="inputSchema is present" if has_schema else "inputSchema is missing (required)",
            spec_reference=f"{self.SPEC_URL}/server/tools",
        ))
    ...
    def _check_name_format(self, tool: Dict[str, Any]) -> List[ComplianceCheck]:
        ...
        checks.append(ComplianceCheck(
            ...
            message="Name format is valid" if is_valid_format else f"Name '{name}' has invalid format",
        ...
        checks.append(ComplianceCheck(
            ...
            message=f"Name length OK ({len(name)} chars)" if is_valid_length else f"Name too long ({len(name)} > {max_length})",
        ))
    ...
    def _check_schema_structure(self, tool: Dict[str, Any]) -> List[ComplianceCheck]:
        ...
        checks.append(ComplianceCheck(
            ...
            message="Root type is 'object'" if is_object else f"Root type must be 'object', got '{root_type}'",
            spec_reference=f"{self.SPEC_URL}/server/tools",
        ))
        ...
        checks.append(ComplianceCheck(
            ...
            message="Schema defines structure" if has_properties else "Schema has no properties defined",
        ))
        ...
        if required and isinstance(required, list):
            ...
            checks.append(ComplianceCheck(
                ...
                message="All required properties are defined" if all_present else f"Missing required properties: {missing}",
            ))
        ...
        for key, expected_type in type_checks.items():
            if key in annotations:
                ...
                checks.append(ComplianceCheck(
                    ...
                    message=f"'{key}' is {expected_type.__name__}" if is_correct_type else f"'{key}' must be {expected_type.__name__}, got {type(value).__name__}",
    ...
    def _check_security_practices(self, tool: Dict[str, Any]) -> List[ComplianceCheck]:
        ...
        checks.append(ComplianceCheck(
            ...
            message="additionalProperties is false" if is_restricted else "additionalProperties should be false for security",
        ...
        destructive_indicators = ["delete", "remove", "drop", "truncate", "destroy", "erase", "clear"]
        appears_destructive = any(ind in name or ind in description for ind in destructive_indicators)
        ...
        if appears_destructive:
            checks.append(ComplianceCheck(
                ...
                message="Destructive tool has destructiveHint annotation" if has_destructive_hint else "Tool appears destructive but lacks destructiveHint annotation",
        ...
        for prop_name, prop_schema in properties.items():
            ...
            if prop_format in ("uri", "url") or "url" in prop_name.lower():
                ...
                checks.append(ComplianceCheck(
                    ...
                    message=f"URL property has pattern validation" if has_pattern else f"URL property '{prop_name}' should have pattern validation",
            ...
            if any(hint in prop_name.lower() for hint in ["file", "path", "directory"]):
                ...
                checks.append(ComplianceCheck(
                    ...
                    level=ComplianceLevel.RECOMMENDED,
                    status=ComplianceStatus.PASS if has_pattern else ComplianceStatus.WARN,
                    message=f"Path property has pattern validation" if has_pattern else f"Path property '{prop_name}' should have pattern validation",
    ...
    def _check_description_quality(self, tool: Dict[str, Any]) -> List[ComplianceCheck]:
        ...
        action_words = ["returns", "gets", "creates", "updates", "deletes", "sends", "reads", "writes", "performs", "executes"]
        ...
        checks.append(ComplianceCheck(
            ...
            message="Description explains tool action" if has_action else "Description should explain what the tool does",
        ))
    ...
    def _check_enterprise_standards(self, tool: Dict[str, Any]) -> List[ComplianceCheck]:
        ...
        checks.append(ComplianceCheck(
            ...
            message="All behavioral hints present" if has_all_hints else f"Missing hints: {[k for k in hint_keys if k not in annotations]}",
        ...
        checks.append(ComplianceCheck(
            ...
            message="$schema declaration present" if has_schema_decl else "Missing $schema declaration",
